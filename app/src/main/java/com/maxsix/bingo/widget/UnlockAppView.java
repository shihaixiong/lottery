package com.maxsix.bingo.widget;
...
import com.maxsix.bingo.vo.PointView;
...
public class UnlockAppView extends View {
	...
	private final int nexusWidth = this.getMeasuredWidth();

	private final int nexusHeight = this.getMeasuredHeight();
				...
				if (password.compareTo(passwordValue) == 0) {
					...
					if (mOnPatternChangeListener != null) {
						...
						if (!TextUtils.isEmpty(passwordValue)) {
							mOnPatternChangeListener.onPatternChange(passwordValue);
						}
					}
					invalidate();// 刷新界面
				}
