...
public class MarxSixStagesActivity extends BaseListActivity {

    private MarxSixStagesAdpter adapter;
    ...
    protected void Refresh()  {
        doAsync(new Callable<com.maxsix.bingo.vo.GridList<com.maxsix.bingo.vo.Stage>>() {
            ...
            public com.maxsix.bingo.vo.GridList<com.maxsix.bingo.vo.Stage> call() throws Exception {
                Gson gson = new Gson();
                String url = com.maxsix.bingo.config.Constants.GetStages_URL + "?" + System.currentTimeMillis() + "&page=" + page + "&status=1&lid=1";
                Log.i("url",url);
            ...
            public void onCallback(com.maxsix.bingo.vo.GridList<com.maxsix.bingo.vo.Stage> pCallbackValue) {
                if (pCallbackValue != null && pCallbackValue.getResults() != null && pCallbackValue.getResults().size()>0) {
                    if(pCallbackValue.getResults().size() <=10) {
                        adapter = new MarxSixStagesAdpter(MarxSixStagesActivity.this, pCallbackValue.getResults());
                    }else{
                        adapter = new MarxSixStagesAdpter(MarxSixStagesActivity.this, pCallbackValue.getResults().subList(0,10));
                    }
